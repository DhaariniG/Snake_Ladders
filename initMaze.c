        ...
        if (sx == ex) {
            for (int y = sy; y <= ey; y++) {
                ...
                {safeSetType(maze, f, sx, y, 1);
                    if(y==ey) fprintf(logfp, "Wall placed from [%d,%d,%d] to [%d,%d,%d]\n", f, sx, sy, ex, ey);
                }
            ...
            for (int x = sx; x <= ex; x++) {
                if (maze[f][x][sy].type == 0) {
                    ...
                    if(x==ex) fprintf(logfp, "Wall placed from [%d,%d,%d] to [%d,%d,%d]\n", f, sx, sy, ex, ey);
                }
...
void addFlag(Cell maze[FLOORS][WIDTH][LENGTH], Flag *flag, const char *filename) {
    ...
    if (fscanf(fp, "[%d,%d,%d]\n", &f, &x, &y) == 3) {
        if (isInBawana(f, x, y)) {
            ...
            fprintf(logfp, "Flag placed at [%d,%d,%d]\n", f, x, y);
        }
    } else {
...
int addStairTarget(Cell maze[FLOORS][WIDTH][LENGTH], int f, int x, int y, int targetFloor, int targetX, int targetY) {
    ...
    if (cell->type == 0 || cell->type == 2) {
        if (cell->numStairs < 2) {
            ...
            cell->numStairs++;
            cell->type = 2;
            fprintf(logfp, "Stair placed from [%d,%d,%d] to [%d,%d,%d]\n", f, x, y, targetFloor, targetX, targetY);
            return 1;
        } 
        else {
...
int selectBestStair(Cell *cell, Flag flag, Stairmode stairMode, int currentFloor) {
    ...
    for (int i = 0; i < cell->numStairs; i++) {
        ...
        if (stairMode == stairs_bi ||
            ...
            fprintf(logfp, "Stair leading to [%d,%d,%d] is chosen.\n",
...
void addPoles(Cell maze[FLOORS][WIDTH][LENGTH], const char *filename) {
    ...
    while (fscanf(fp, "[%d,%d,%d,%d]\n", &lf, &hf, &x, &y) == 4) {
        ...
        for (int f = hf; f >= lf; f--) {
            ...
            if(f == lf) fprintf(logfp, "Pole placed from [%d,%d,%d] to [%d,%d,%d]\n", hf,x,y,lf,x,y);
        ...
        for (int x = 0; x < WIDTH; x++) {
            for (int y = 0; y < LENGTH; y++) {
                ...
                if (r < 25) { 
                    ...
                    cell->consumable = (rand() % 4) + 1; 
                }
                else if (r < 85) { 
                    cell->bonusAdd = (rand() % 2) + 1; 
                }
                else if (r < 95) { 
                    cell->bonusAdd = (rand() % 3) + 3;
                }
                else { 
                    cell->bonusMul = (rand() % 2) ? 3 : 2; 
                }
            }
        }
    }
